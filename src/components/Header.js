...
export default function Header() {
  ...
  const { user } = useAuth() || {};           // no sign-up / sign-in now
      ...
      <nav>
        <div className="nav-links-n-logo">
          <Link href={brandHref} className="brand" aria-label="Effy Exotics Home">
            <Image src="/img/effy-dispensary.svg" alt="Effy Exotics logo" width={140} height={40} priority />
          </Link>

          {/* CITY SWITCHER */}
          <div className="location-switcher" style={{ fontFamily:"Legend-M54", marginLeft:24 }}>
            <button
              onClick={() => switchTo("las-cruces")}
              style={{ background:"none",border:0,color: city==="las-cruces" ? "#C09B31" : "#fff",cursor:"pointer" }}
            >Las Cruces</button>{" | "}
            <button
              onClick={() => switchTo("alamogordo")}
              style={{ background:"none",border:0,color: city==="alamogordo" ? "#C09B31" : "#fff",cursor:"pointer" }}
            >Alamogordo</button>
          </div>
        ...
        <button className="menu-btn" aria-label="Open menu" onClick={() => setOpen(true)}>
          <span className="hamburger"><span className="line" /><span className="line" /><span className="line" /></span>
      ...
      <div className={`mobile-menu${open ? " active" : ""}`} aria-hidden={!open}>
        ...
        <ul className="menu-list">
          ...
          <li style={{ marginTop:32 }}>
            <button onClick={() => switchTo("las-cruces")}  style={{ background:"none",border:0,color:"#fff",cursor:"pointer" }}>Las Cruces</button>{" | "}
            <button onClick={() => switchTo("alamogordo")} style={{ background:"none",border:0,color:"#fff",cursor:"pointer" }}>Alamogordo</button>
          </li>
